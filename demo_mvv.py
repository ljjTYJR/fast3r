...
def main():
    ...
    p.add_argument("--cameras", nargs='+', default=None, help="Camera subdirs to process (auto-detect if not set)")
    ...
    p.add_argument("--sampling_interval", type=int, default=1, help="Use every Nth frame")
    ...
    # Auto-detect camera directories if not specified
    if args.cameras is None:
        print(f"\nAuto-detecting cameras in: {args.data_dir}")
        all_subdirs = [d for d in os.listdir(args.data_dir) if os.path.isdir(os.path.join(args.data_dir, d))]
        cam_dirs = sorted([d for d in all_subdirs if d.startswith('cam') and d[3:].isdigit()])
        if not cam_dirs:
            print("No camera directories found (looking for 'camXX' pattern)")
            return
        args.cameras = cam_dirs
        print(f"Found {len(cam_dirs)} cameras: {', '.join(cam_dirs)}")
